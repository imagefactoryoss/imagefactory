...
const SRESmartBotOperatorRulesSettingsPage: React.FC = () => {
    ...
    const loadPolicy = async () => {
        try {
            ...
            setError(err instanceof Error ? err.message : 'Failed to load operator rules settings')
        } finally {
            setLoading(false)
        }
    }
    ...
    const savePolicy = async () => {
        ...
        try {
            ...
            setPolicy(normalizePolicy(response))
            toast.success('Operator rules saved')
        } catch (err) {
            const message = err instanceof Error ? err.message : 'Failed to save operator rules'
            setError(message)
            toast.error(message)
        } finally {
            setSaving(false)
    ...
    const persistOperatorRule = async (index: number, updates: Partial<SRESmartBotOperatorRule>) => {
        let nextPolicy: SRESmartBotPolicyConfig | null = null
        setPolicy((current) => {
            if (!current) return current
            const operatorRules = [...current.operator_rules]
            operatorRules[index] = { ...operatorRules[index], ...updates }
            nextPolicy = { ...current, operator_rules: operatorRules }
            return nextPolicy
        })
        ...
        try {
            ...
            setPolicy(normalizePolicy(response))
            toast.success('Operator rule saved')
        } catch (err) {
            const message = err instanceof Error ? err.message : 'Failed to persist operator rule update'
            setError(message)
            toast.error(message)
            void loadPolicy()
        } finally {
            ...
            <div className="w-full space-y-6">
                <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
                    <div>
                        ...
                        <h1 className="mt-2 text-3xl font-semibold tracking-tight">SRE Smart Bot Operator Rules</h1>
                        <p className="mt-2 max-w-4xl text-sm text-slate-600 dark:text-slate-400">
                            Manage operator-defined escalation and routing behavior on a dedicated page optimized for larger rule sets.
                        </p>
                    </div>
                ...
                <SRESmartBotRuleSettingsNav active="operator" />
                ...
                {loading ? (
                    ...
                    <RulesSettingsPanel
                        ...
                        showOperatorRules
                        showDetectorRules={false}
                        showTabIntro={false}
                        operatorMutationInFlight={ruleMutationInFlight}
                        onOperatorRuleSave={(index, updates) => {
                            void persistOperatorRule(index, updates)
                        }}
                    />
                ) : null}
            </div>
...
export default SRESmartBotOperatorRulesSettingsPage
