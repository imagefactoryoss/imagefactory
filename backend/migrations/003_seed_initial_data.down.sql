...
DELETE FROM user_role_assignments WHERE user_id IN (
    SELECT id FROM users WHERE email IN ('admin@imagefactory.local', 'system@imagefactory.local')
);

DELETE FROM roles WHERE is_system_role = true;

DELETE FROM users WHERE email IN ('admin@imagefactory.local', 'system@imagefactory.local');
