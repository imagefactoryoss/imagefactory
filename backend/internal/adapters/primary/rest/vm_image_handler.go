...
func (h *VMImageHandler) ListTenantVMImages(w http.ResponseWriter, r *http.Request) {
	...
	for _, row := range rows {
		targetProvider, targetProfileID, providerIdentifiers := parsePackerMetadataFields(row.MetadataRaw)
		...
		items = append(items, vmImageCatalogItem{
			...
			LifecycleState:              vmImageLifecycleState(row.ExecutionStatus),
		})
	}
...
func vmImageLifecycleState(executionStatus string) string {
...
func parsePackerMetadataFields(raw json.RawMessage) (targetProvider, targetProfileID string, providerIdentifiers map[string][]string) {
	...
	return targetProvider, targetProfileID, out
}
