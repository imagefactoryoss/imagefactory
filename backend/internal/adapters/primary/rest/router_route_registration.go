...
func registerCoreAPIRoutes(
	...
	router.Post("/api/v1/images/vm/{executionId}/promote", tenantStatusMiddleware.EnforceTenantStatus(authMiddleware.RequirePermission(permissionService, "image", "update")(http.HandlerFunc(vmImageHandler.PromoteTenantVMImage))).ServeHTTP)
	router.Post("/api/v1/images/vm/{executionId}/deprecate", tenantStatusMiddleware.EnforceTenantStatus(authMiddleware.RequirePermission(permissionService, "image", "update")(http.HandlerFunc(vmImageHandler.DeprecateTenantVMImage))).ServeHTTP)
	router.Delete("/api/v1/images/vm/{executionId}", tenantStatusMiddleware.EnforceTenantStatus(authMiddleware.RequirePermission(permissionService, "image", "delete")(http.HandlerFunc(vmImageHandler.DeleteTenantVMImage))).ServeHTTP)
