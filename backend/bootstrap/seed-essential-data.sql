...
-- ============================================================================
-- SEED GLOBAL TOOL AVAILABILITY CONFIG (required for tools management UI/API)
-- ============================================================================
...
SELECT
  ...
  'tool_availability',
  '{
    "build_methods":{"container":true,"packer":true,"paketo":true,"kaniko":true,"buildx":true,"nix":true},
    "sbom_tools":{"syft":true,"grype":true,"trivy":true},
    "scan_tools":{"trivy":true,"clair":false,"grype":true,"snyk":false},
    "registry_types":{"s3":true,"harbor":false,"quay":false,"artifactory":false},
    "secret_managers":{"vault":false,"aws_secretsmanager":true,"azure_keyvault":false,"gcp_secretmanager":false},
    "trivy_runtime":{
      "cache_mode":"shared",
...
(NULL, 'System Administrator Viewer', 'Read-only system-wide access for administrative monitoring', true, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP),
(NULL, 'Security Reviewer', 'Central security/governance approver for quarantine workflows', true, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP),
...
-- Quarantine approval governance
('quarantine', 'read', 'Read quarantine import request workflow state', 'quarantine_management', true, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP),
('quarantine', 'approve', 'Approve quarantine import requests', 'quarantine_management', true, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP),
('quarantine', 'reject', 'Reject quarantine import requests', 'quarantine_management', true, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP),
('quarantine', 'release', 'Release quarantine-approved artifacts for tenant consumption', 'quarantine_management', true, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)
...
-- ============================================================================
-- ASSIGN PERMISSIONS TO SECURITY REVIEWER ROLE (essential - central approvals)
-- ============================================================================
INSERT INTO role_permissions (role_id, permission_id, granted_at)
SELECT r.id, p.id, CURRENT_TIMESTAMP
...
WHERE r.name = 'Security Reviewer'
  ...
  AND r.tenant_id IS NULL
  AND (
    (p.resource = 'quarantine' AND p.action IN ('read', 'approve', 'reject', 'release'))
...
-- ============================================================================
-- ASSIGN PERMISSIONS TO SYSTEM ADMINISTRATOR VIEWER ROLE (essential - system read-only)
-- ============================================================================
INSERT INTO role_permissions (role_id, permission_id, granted_at)
SELECT r.id, p.id, CURRENT_TIMESTAMP
FROM rbac_roles r, permissions p
WHERE r.name = 'System Administrator Viewer'
  AND r.is_system = true
  AND r.tenant_id IS NULL
  AND (
    (p.resource = 'system' AND p.action IN ('read'))
    OR (p.resource = 'tenant' AND p.action IN ('read', 'list'))
    OR (p.resource = 'user' AND p.action IN ('read', 'list'))
    OR (p.resource = 'role' AND p.action IN ('read', 'list'))
    OR (p.resource = 'permissions' AND p.action IN ('read'))
    ...
    OR (p.resource = 'analytics' AND p.action IN ('read'))
    OR (p.resource = 'workers' AND p.action IN ('read'))
    OR (p.resource = 'queue' AND p.action IN ('read'))
    OR (p.resource = 'groups' AND p.action IN ('read'))
    OR (p.resource = 'invitation' AND p.action IN ('list'))
    OR (p.resource = 'mfa' AND p.action IN ('read'))
    OR (p.resource = 'sso' AND p.action IN ('read'))
    OR (p.resource = 'config' AND p.action IN ('read'))
  )
ON CONFLICT (role_id, permission_id) DO NOTHING;
...
WHERE r.name = 'Owner' AND r.is_system = true AND r.tenant_id IS NULL
  AND (
    ...
    -- System read access
    OR (p.resource = 'system' AND p.action IN ('read'))
    -- Quarantine request visibility for tenant workflows
    OR (p.resource = 'quarantine' AND p.action IN ('read'))
  )
ON CONFLICT (role_id, permission_id) DO NOTHING;
